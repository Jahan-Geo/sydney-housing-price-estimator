...
if st.button("Estimate sale price", use_container_width=True):
    # NOTE: only RAW fields are supplied here. The fitted pipeline's HousingFeatureEngineer
    # step (trained on all 179 properties, see Section 6.2) handles all imputation and
    # feature derivation automatically -- unknown land size / floor area are passed as
    # np.nan and imputed using the same training-time group-median logic the model was
    # fit with, rather than a hardcoded placeholder value.
    row = {
        "suburb": suburb,
